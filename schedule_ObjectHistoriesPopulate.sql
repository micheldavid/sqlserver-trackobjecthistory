...
EXEC @ReturnCode =  dbo.sp_add_job @job_name=N'_DB ObjectHistoriesPopulate', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [ObjectHistoriesPopulate]    Script Date: 4/29/2014 10:54:46 AM ******/
EXEC @ReturnCode = dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'ObjectHistoriesPopulate', 
		@step_id=1, 
		...
		@command=N'EXEC [dbo].[ObjectHistoriesPopulate]', 
		@database_name=N'_Maintenance', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
DECLARE @schedule_id int
EXEC @ReturnCode = dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Hourly', 
		@enabled=1,
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=8, 
